...
class AlumnosModel extends CI_Model {
    ...
    {
        $this->db->select('al.id, al.nombre_al, al.apellido_al, al.fecha_nac, al.sexo, al.estados_id, al.municipios_id, al.grado_id, al.seccion_id, al.anio_id, g.grado, s.seccion, ae.anio, e.estado, m.municipio');
    ...
    {
        $this->db->select('al.id, al.nombre_al, al.apellido_al, al.fecha_nac, al.sexo, al.estados_id, al.municipios_id, e.estado, m.municipio');
        $this->db->from($this->tabla.' AS al');
        $this->db->join('estados AS e', 'e.id=al.estados_id', 'inner');
        $this->db->join('municipios AS m', 'm.id=al.municipios_id', 'inner');
